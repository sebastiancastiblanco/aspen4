class ReportesController < ApplicationController
  def index
  	...
  	@proceso = current_abogado.procesos.order('id,created_at DESC').last
    ...
    if (!@proceso.nil?)
       @procesoLog = Log.where(proceso_id: @proceso.id ).order('id, updated_at DESC').limit(5)
      @alertasPendientes = @proceso.alertas.where("termina < ?", Time.now).order("id,updated_at DESC").limit(4);
      @actividades = @proceso.actividads.activos.order('id,updated_at DESC').limit(2)
      @documentos = @proceso.documents.activos.order('id,updated_at DESC').limit(1)
