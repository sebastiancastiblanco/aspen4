class ContratosController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @contrato.save
        #Traza de log
        Log.create(:usuario => current_abogado.email,:proceso => @proceso.tipo_proceso.tipo+' - '+@proceso.titulo ,:abogado_id => current_abogado.id ,:proceso => @proceso, :contrato_id => @contrato.id, :mensaje_id => 3 ,:mensaje=> current_abogado.email.to_s+', Creo el contrato: '+@contrato.objeto)
  ...
  def update
    ...
    respond_to do |format|
      if @contrato.update_attributes(params[:contrato])
        #Traza de log
        Log.create(:usuario => current_abogado.email,:proceso => @proceso.tipo_proceso.tipo+' - '+@proceso.titulo ,:abogado_id => current_abogado.id ,:proceso => @proceso, :contrato_id => @contrato.id, :mensaje_id => 3 ,:mensaje=> current_abogado.email.to_s+', Modifico el contrato: '+@contrato.objeto)
  ...
  def destroy
    ...
    respond_to do |format|
      #Traza de log
      Log.create(:usuario => current_abogado.email,:proceso => @proceso.tipo_proceso.tipo+' - '+@proceso.titulo ,:abogado_id => current_abogado.id ,:proceso => @proceso, :contrato_id => @contrato.id, :mensaje_id => 3 ,:mensaje=> current_abogado.email.to_s+', Elimino el contrato: '+@contrato.objeto)
