class EventosController < ApplicationController
     ...
     zone = ActiveSupport::TimeZone.new("America/Bogota")
      ...
      respond_to do |format|
        if @evento.save
          ...
          @evento.usuario_eventos.create(:abogado_id => current_abogado.id,:propietario_id => current_abogado.id,:evento_id => @evento.id)
  ...
  def move
    ...
    if @evento
      @evento.start = (params[:minute_delta].to_i).minutes.from_now((params[:day_delta].to_i).days.from_now(@evento.start))
  ...
  def get_events
    #@eventos = Event.find(:all, :conditions => ["starttime >= '#{Time.at(params['start'].to_i).to_formatted_s(:db)}' and endtime <= '#{Time.at(params['end'].to_i).to_formatted_s(:db)}'"] )
    @eventos = Evento.all 
    events = [] 
